...
contract Date is Ownable, ERC721
{
    ...
    {  
        setBaseURI("https://localhost/token");
    ...
    function setBaseURI(string memory baseURI) public onlyOwner { _currentBaseURI = baseURI; }

    function _baseURI() internal view virtual override returns (string memory) { return _currentBaseURI; }
    ...
    function claim(uint16 year, uint8 month, uint8 day, string calldata title) external payable
    {
        ...
        if      (r < 1000)   { color = 7; } 
        else if (r < 6000)   { color = 6; }
        else if (r < 16000)  { color = 5; }
        else if (r < 66000)  { color = 4; }
        else if (r < 166000) { color = 3; }
        else if (r < 366000) { color = 2; }
        else if (r < 666000) { color = 1; }
        else                 { color = 0; }
    ...
    function ownerOf(uint16 year, uint8 month, uint8 day) public view returns(address) { return ownerOf(id(year, month, day)); }
